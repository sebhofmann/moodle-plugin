<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="courseContent"
                select="document(concat('moodle:resolveCourseContent:',/RESPONSE/MULTIPLE/SINGLE/KEY[@name='id']/VALUE/text()))"/>
  ...
  <xsl:template match="SINGLE">
    <mods:mods>
      ...
      <xsl:for-each select="distinct-values($courseContent//KEY[@name='author']/VALUE/text())">
  ...
  <xsl:template match="KEY[@name='fullname' and count(VALUE/node())&gt;0]">
